  ...
  try {
    ...
    if (agentResult.success) {
      ...
      if (pushResult.pushFailed) {
        ...
        if (autoCreatePR && task.branchName && task.repoUrl) {
          try {
            await logger.info('Creating pull request automatically')
            
            const prTitle = prompt.length > 72 ? prompt.substring(0, 69) + '...' : prompt
            const prBody = `This pull request was automatically generated by the AI agent.\n\n**Task:** ${prompt}`
            ...
            if (prResult.success) {
              ...
              await logger.success('Pull request created successfully')
              console.log('Auto-created PR:', prResult.prUrl)
            } else {
              await logger.error('Failed to create pull request')
              console.error('Failed to auto-create PR:', prResult.error)
            }
          } catch (prError) {
            console.error('Error auto-creating PR:', prError)
            await logger.error('Error creating pull request')
          }
        }
      }
    } else {
