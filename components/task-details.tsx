        ...
        <div className="flex items-center gap-2 md:gap-4 flex-wrap text-xs md:text-sm">
          ...
          {task.repoUrl && (
            <div className="flex items-center gap-1.5 md:gap-2 min-w-0">
              <svg className="h-3.5 w-3.5 md:h-4 md:w-4 flex-shrink-0 text-muted-foreground" fill="currentColor" viewBox="0 0 24 24">
                <path
                  fillRule="evenodd"
          ...
          {(task.selectedAgent || task.selectedModel) && (
            <div className="flex items-center gap-1.5 md:gap-2">
              ...
              {task.selectedModel && (
                <span className="text-muted-foreground truncate">{getModelName(task.selectedModel, task.selectedAgent)}</span>
              )}
            </div>
          )}
