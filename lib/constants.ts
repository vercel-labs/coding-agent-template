...
export const VERCEL_DEPLOY_URL =
  'https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fvercel-labs%2Fcoding-agent-template&env=VERCEL_TEAM_ID,VERCEL_PROJECT_ID,VERCEL_TOKEN,JWE_SECRET,ENCRYPTION_KEY&envDescription=Required+environment+variables+for+the+coding+agent+template.+Database+will+be+automatically+provisioned+via+Neon.+You+must+also+configure+at+least+one+OAuth+provider+(GitHub+or+Vercel).+Optional+API+keys+can+be+added+later+in+project+settings.&project-name=coding-agent-template&repository-name=coding-agent-template'
